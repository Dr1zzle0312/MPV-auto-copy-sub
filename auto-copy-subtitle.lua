...
local use_zh_osd = false
local lang_prop = mp.get_property("language", "")
if lang_prop:find("^zh") then
    use_zh_osd = true
end

local function tr(zh_txt, en_txt)
    if use_zh_osd then
        return zh_txt
    else
        return en_txt
...
local function manual_copy_subtitle()
    ...
    if subtitle and subtitle ~= "" then
        ...
        mp.osd_message(tr("✅ 全屏字幕已复制到剪贴板", "✅ Full subtitles copied to clipboard"), 1.5)
    else
        mp.osd_message(tr("⚠️ 当前无字幕", "⚠️ No subtitle available"), 1.5)
    end
end
-- 切换自动复制功能
local function toggle_auto_copy()
    ...
    if auto_copy_enabled then
        ...
        mp.osd_message(tr("🟢 自动复制已启用 (全部字幕行+去重+关键词屏蔽)", "🟢 Auto-copy enabled (all lines + dedup + keyword filter)"), 2)
    else
        if observer_id then
            mp.unobserve_property(observer_id)
            observer_id = nil
        end
        mp.osd_message(tr("🔴 自动复制已禁用", "🔴 Auto-copy disabled"), 2)
    end
end
-- 切换视频时重置状态
...
-- 初始状态提示
mp.osd_message(tr("📌 字幕复制脚本已加载\n" ..
              "• Ctrl+C = 手动复制全屏所有字幕\n" ..
              "• Ctrl+Shift+C = 开启自动复制",
              "📌 Subtitle script loaded\n• Ctrl+C = Manual copy all subtitles\n• Ctrl+Shift+C = Toggle auto-copy"), 3)
